...
const UserController = {
  ...
  async register(req, res) {
    try {
      const { username, password, email, firstName, lastName, age, address } = req.body;

      const { error } = registerSchema.validate({ username, password, email, firstName, lastName, age, address });
      if (error) {
        const errorMessage = error.details.map((detail) => detail.message).join(', ');
        return res.status(400).json({ error: errorMessage, body: req.body });
      }
